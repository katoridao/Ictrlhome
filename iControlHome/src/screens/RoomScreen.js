...
import {
  ...
  ScrollView, Modal, TextInput, Alert, ActivityIndicator, TouchableWithoutFeedback
} from 'react-native';
...
export default function RoomScreen({ navigation }) {
  ...
  const [togglingRoomId, setTogglingRoomId] = useState(null); // ID phòng đang tắt tổng
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit && selectedRoomId) {
        ...
        if (!houseId) { Alert.alert("Lỗi", "Vui lòng chọn nhà trước."); return; }
    ...
    Alert.alert("Xác nhận", "Xóa phòng này?", [
      { text: "Hủy", style: "cancel" },
      {
        text: "Xóa", style: "destructive", onPress: async () => {
          try { await api.delete(`/rooms/del/${id}`); fetchRooms(); }
