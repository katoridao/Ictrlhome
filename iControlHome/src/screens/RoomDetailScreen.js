...
export default function RoomDetailScreen({ route, navigation }) {
  ...
  // Chỉ tính trên thiết bị mà user có quyền điều khiển
  ...
  const selectAll = () => {
    // Chỉ cho chọn thiết bị có quyền điều khiển
  ...
  const handleToggleAll = async () => {
    ...
    setTogglingAll(true);
    try {
      // Chỉ gửi API cho thiết bị chưa ở đúng trạng thái cần chuyển
      const needsUpdate = controllableDevices.filter(d => !!d.status !== newStatus);
      if (needsUpdate.length === 0) return; // Tất cả đã đúng trạng thái, không làm gì
      ...
      Alert.alert("Lỗi", "Không thể cập nhật tất cả thiết bị");
    } finally {
  ...
  const handleToggleSelected = async (newStatus) => {
    ...
    try {
      ...
      Alert.alert("Lỗi", "Không thể cập nhật thiết bị đã chọn");
    } finally {
      ...
      { text: "Hủy", style: "cancel" },
      {
        text: "Gỡ bỏ", style: "destructive", onPress: async () => {
          try {
            ...
            Alert.alert("Lỗi", "Không thể gỡ thiết bị.");
          }
        }
      }
    ]);
  ...
  const renderDeviceItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          if (selectMode) {
            if (hasPermission) toggleSelectItem(item._id); // Chỉ chọn thiết bị có quyền
            return;
          }
...
const styles = StyleSheet.create({
  ...
  quickBtn: {
    ...
    borderRadius: 10, gap: 6, justifyContent: 'center',
  },
  ...
  selectHeader: {
    ...
    borderRadius: 14, paddingHorizontal: 14, paddingVertical: 10, marginBottom: 10, elevation: 2,
  },
