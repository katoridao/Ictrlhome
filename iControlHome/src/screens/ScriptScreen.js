      ...
      >
        {loading && !refreshing ? (
          ...
          <View style={styles.emptyContainer}>
            <Text style={{ color: '#999', fontSize: 16 }}>Chưa có kịch bản tự động nào</Text>
          </View>
        )}
      </ScrollView>
    </View>
...
function ScriptItem({ item, themeStyles, onDelete }) {
  ...
  return (
    <View style={[styles.card, { backgroundColor: themeStyles.card }]}>
      <View style={{ flex: 1 }}>
        <Text style={[styles.cardText, { color: themeStyles.text }]}>{item.name}</Text>
        
        <View style={styles.infoRow}>
            <Text style={styles.subLabel}>Thiết bị: </Text>
            <Text style={[styles.valText, {color: themeStyles.text}]}>{item.device_id?.name || "N/A"}</Text>
        </View>

        <View style={styles.infoRow}>
            <Text style={styles.subLabel}>Lịch trình: </Text>
            <Text style={[styles.timeHighlight, { color: themeStyles.primary }]}>⏰ {item.trigger_time}</Text>
            <View style={[styles.badge, { backgroundColor: isON ? '#E8F5E9' : '#FFEBEE' }]}>
                <Text style={{ color: isON ? '#2E7D32' : '#C62828', fontSize: 10, fontWeight: 'bold' }}>{isON ? 'BẬT' : 'TẮT'}</Text>
            </View>
        </View>

        <View style={styles.footerRow}>
             <Image source={require('../../public/img/user.png')} style={styles.userIcon} />
             <Text style={styles.creatorName}>Người tạo: {item.user_id?.name || "Hệ thống"}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  body: { padding: 16 },
  card: { borderRadius: 16, padding: 16, marginBottom: 15, flexDirection: 'row', alignItems: 'center', elevation: 2, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 5 },
  ...
  footerRow: { flexDirection: 'row', alignItems: 'center', marginTop: 10, paddingTop: 10, borderTopWidth: 0.5, borderTopColor: '#EEE' },
